...
namespace ChurchManager.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { UserName = model.Email, Email = model.Email, OwnerGroupId=Guid.NewGuid(), IsAccountOwner=true  };
        ...
        public async Task<ActionResult> ExternalLoginConfirmation(ExternalLoginConfirmationViewModel model, string returnUrl)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    //Add an admin Role by default for new user
                    AddDefaultRole(user);
                    ...
                    if (result.Succeeded)
                    {
                        await SignInManager.SignInAsync(user, isPersistent: false, rememberBrowser: false);
                        //return RedirectToLocal(returnUrl);
                        return RedirectToAction("Create", "Churches");
                    }
        ...
        private void AddDefaultRole(ApplicationUser user) {
            ...
            {
                Id = roleId,
                Name = string.Format("{0}.Admin", user.OwnerGroupId),
                Description = "Admin",
                OwnerGroupId = user.OwnerGroupId,
                IsDefaultRole = true,
                Operations = context.Operations.ToList().Select(x => new RoleOperation() { RoleId = roleId, OperationId = x.Id }).ToList()
            };
